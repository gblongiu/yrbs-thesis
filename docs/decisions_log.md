...
- Rationale: Outcome-missing rows cannot support supervised training/evaluation.
- Evidence:
  - `scripts/01_build_dataset.py`
...
- Rationale: Week 4 uses survey weights in EDA because descriptive prevalence should reflect the YRBS sampling design. The baseline predictive workflow remains unweighted so that model fitting and held-out evaluation are directly comparable under one fixed protocol and consistent with the Week 4 baseline objective. This separation also avoids mixing descriptive weighting decisions into the primary predictive benchmark before robustness checks are defined. Weighted-fit modeling is deferred to planned sensitivity work so it can be evaluated explicitly as a robustness branch rather than merged into the primary Week 4 analysis.
- Evidence:
  - `docs/eda_report.md`
  - `docs/modeling_report.md`
